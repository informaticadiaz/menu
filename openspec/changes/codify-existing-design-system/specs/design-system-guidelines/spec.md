...
El sistema de diseño SHALL tomar `/`, `/admin/login` y `/admin` como canon inicial de la identidad visual de Fuego.

#### Scenario: Nueva pantalla usa el canon existente
- **WHEN** se diseña o implementa una nueva pantalla del frontend
- **THEN** su fondo, tarjetas, tipografía, acentos, botones y estados coinciden visualmente con los patrones existentes en `/`, `/admin/login` y `/admin`
